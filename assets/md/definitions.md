...
$$ \begin{align} Lift \bigl( A \Rightarrow B \bigr) & \equiv \frac{Conf \bigl( A \Rightarrow B \bigr) }{Supp(B)} \\\ & = \frac{P \bigl( B \; | \; A \bigr) }{P(B)} = \frac{P \bigl( A \cap B \bigr)}{P(A)P(B)} = \frac{P \bigl( B \cap A \bigr)}{P(A)P(B)} \\\ & = \frac{Conf \bigl( B \Rightarrow A \bigr) }{Supp(A)} = Lift \bigl( B \Rightarrow A \bigr) \end{align} $$
